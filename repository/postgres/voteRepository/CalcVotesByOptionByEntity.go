...
func (d DB) CalcVotesByOptionByEntity(entityType string, entityID int) ([]entity.OptionVoteResult, error) {

	column, ok := allowedEntityColumns[entityType]
	if !ok {
		return nil, richerror.New("voteRepository-CalcVotesByOptionByEntity").
	...
	if err != nil {
		return nil, richerror.New("voteRepository-CalcVotesByOptionByEntity").WithErr(err).WithKind(richerror.KindUnexpected)
	}
	defer rows.Close()
